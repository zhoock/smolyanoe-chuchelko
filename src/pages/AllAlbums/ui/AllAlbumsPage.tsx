...
export function AllAlbumsPage() {
  ...
  // SEO
  const seoTitle = lang === 'en' ? 'All Albums' : 'Все альбомы';
  const seoDesc =
    lang === 'en' ? 'Browse all albums and releases' : 'Просмотр всех альбомов и релизов';
          ...
          <>
            ...
            {!hasMore && allAlbums.length > 0 && (
              <p className="all-albums__end">
                {lang === 'en' ? 'All albums loaded' : 'Все альбомы загружены'}
              </p>
            )}
          </>
        )}
