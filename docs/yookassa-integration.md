...
### 2. Настройка окружения (Netlify / `.env`)

Для платёжных функций **обязательны** только:
...
**`YOOKASSA_SHOP_ID` и `YOOKASSA_SECRET_KEY` не задаются в Netlify для оплаты** — они сохраняются на артиста в БД через настройки платежей в ЛК сайта (`POST /api/payment-settings`).

**Важно:** На стороне ЮKassa каждый артист использует свои ключи (тестовые или боевые) при первом сохранении в ЛК).
